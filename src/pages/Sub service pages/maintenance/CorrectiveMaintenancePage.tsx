import React from 'react';
import SubServicePageLayout from '../../SubServicePageLayout';
import { correctiveMaintenancePageData } from '../../../sub-service-data/maintenance/correctiveMaintenanceData';

// --- IMPORTANT ---
// The keys used here (e.g., 'scopeImage') MUST match the keys in your data file.

// Standardized scope and testimonial images from the 'maintenance' folder
import scopeImage from '../../../assert/Sub Services/maintenance/smart.webp'; 
import testimonialImage from '../../../assert/Sub Services/maintenance/Maintenance-testimonials-7.jpg';

// Importing the specific project images you requested.
import projectImg1 from '../../../assert/projects/pro (45).png';
import projectImg2 from '../../../assert/projects/pro (50).png';
import projectImg3 from '../../../assert/projects/pro (15).png';
import projectImg4 from '../../../assert/projects/pro (23).png';
import projectImg5 from '../../../assert/projects/pro (15).png';
import projectImg6 from '../../../assert/projects/pro (110).png';
import projectImg7 from '../../../assert/projects/pro (103).png';

const imageAssets = {
  scopeImage,
  testimonialImage,
  projectImg1,
  projectImg2,
  projectImg3,
  projectImg4,
  projectImg5,
  projectImg6,
  projectImg7,
};

const CorrectiveMaintenancePage = () => {
  return (
    <SubServicePageLayout 
      pageData={correctiveMaintenancePageData} 
      imageAssets={imageAssets}
    />
  );
};

export default CorrectiveMaintenancePage;