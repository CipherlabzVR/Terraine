// Suggested Filename: PolymerCompositeConsultancy.tsx

import React from 'react';
import SubServicePageLayout from '../../SubServicePageLayout';
import { polymerCompositeConsultancyPageData } from '../../../sub-service-data/material/polymerCompositeConsultancyData';

// NOTE: These keys MUST match the keys used in polymerCompositeConsultancyPageData
// (scopeImage, testimonialImage, projectImg1..projectImg7)

// Shared/standard images – swap with polymer/composite visuals when ready
import scopeImage from '../../../assert/Sub Services/material/Material-testimonials-14.jpg';
import testimonialImage from '../../../assert/Sub Services/material/Material-testimonials-17.jpg';

import projectImg1 from '../../../assert/projects/pro (13).png';
import projectImg2 from '../../../assert/projects/pro (54).png';
import projectImg3 from '../../../assert/projects/pro (35).png';
import projectImg4 from '../../../assert/projects/pro (99).png';
import projectImg5 from '../../../assert/projects/pro (41).png';
import projectImg6 from '../../../assert/projects/pro (55).png';
import projectImg7 from '../../../assert/projects/pro (19).png';

const imageAssets = {
  scopeImage,
  testimonialImage,
  projectImg1,
  projectImg2,
  projectImg3,
  projectImg4,
  projectImg5,
  projectImg6,
  projectImg7
};

const PolymerCompositeConsultancyPage: React.FC = () => {
  return (
    <SubServicePageLayout
      pageData={polymerCompositeConsultancyPageData}
      imageAssets={imageAssets}
    />
  );
};

export default PolymerCompositeConsultancyPage;
